...
export const nodeEditorCode = (scene: BABYLON.Scene) => {
  const nodeMaterial = new BABYLON.NodeMaterial("nodeEditorCode", scene);
  ...
  // InputBlock
  const Float1 = new BABYLON.InputBlock("Float1");
  ...
  // OneMinusBlock
  const inBounds = new BABYLON.OneMinusBlock("inBounds");
  ...
  // StepBlock
  const upperBounds = new BABYLON.StepBlock("upperBounds");
  upperBounds.visibleInInspector = false;
  upperBounds.visibleOnFrame = false;
  upperBounds.target = 4;

  // InputBlock
  const ceilingValue = new BABYLON.InputBlock("ceilingValue");
  ...
  // InputBlock
  const ceilingColor = new BABYLON.InputBlock("ceilingColor");
  ceilingColor.visibleInInspector = false;
  ceilingColor.visibleOnFrame = false;
  ceilingColor.target = 1;
  ceilingColor.value = new BABYLON.Color3(
    0.30196078431372547,
    0.32941176470588235,
    0.8705882352941177
  );
  ...
  // StepBlock
  const lowerBounds = new BABYLON.StepBlock("lowerBounds");
  ...
  // InputBlock
  const floorValue = new BABYLON.InputBlock("floorValue");
